...
func (gatewayObject *gatewayTowardsPluginObject_struct) databaseEngine() {
	...
	for {
		...
		messageToDbEngine := <-gatewayObject.gatewayCommonObjects.dbMessageQueue
		gatewayObject.gatewayCommonObjects.logger.WithFields(logrus.Fields{
		...
		case DB_READ:
			...
			err = gatewayObject.gatewayCommonObjects.db.View(func(tx *bolt.Tx) error {
		...
		case DB_WRITE:
			...
			err = gatewayObject.gatewayCommonObjects.db.Update(func(tx *bolt.Tx) error {

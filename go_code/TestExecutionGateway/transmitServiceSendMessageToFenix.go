...
func (gatewayObject *GatewayTowardsPluginObject_struct) initiateSendMessageToFenix() {
	...
	go gatewayObject.dispatchEngineForSendMessageToFenix()
}
...
func (gatewayObject *GatewayTowardsPluginObject_struct) dispatchEngineForSendMessageToFenix() {
	...
	for {
		...
		if err != nil {
			...
			if err != nil {
				...
				gatewayClient := gRPC.NewGatewayTowardsFenixClient(remoteChildServerConnection)
				...
				if err != nil {
					...
					gatewayObject.logger.WithFields(logrus.Fields{
						...
						"addressToDial": addressToDial,
					}).Debug("gRPC-send OK of 'informationMessageToBeForwarded' to parent-Gateway or Plugin")
